...
import { Wrench, ArrowLeft, AlertCircle, Leaf } from "lucide-react"
...
export default function WorkerLogin() {
  ...
  const handleLogin = async (e) => {
    ...
    setTimeout(() => {
      // Check if worker exists in registration data
      const registeredUsers = JSON.parse(localStorage.getItem("registeredUsers") || "[]")
      const worker = registeredUsers.find((u) => u.email === email && u.role === "worker")

      if (worker && worker.password === password) {
        localStorage.setItem("userRole", "worker")
        localStorage.setItem("userEmail", email)
        localStorage.setItem("userName", worker.name || "Worker")
        router.push("/dashboard/worker")
      } else {
        // Allow demo login for testing
        if (email === "worker@civicx.com" && password === "worker123") {
          localStorage.setItem("userRole", "worker")
          localStorage.setItem("userEmail", email)
          localStorage.setItem("userName", "Demo Worker")
          router.push("/dashboard/worker")
        } else {
          setError("Invalid credentials. Try worker@civicx.com / worker123 for demo access.")
        }
      }
      setIsLoading(false)
    }, 1000)
          ...
          <CardContent>
            ...
            <div className="mt-4 p-3 bg-muted rounded-md">
              <p className="text-xs text-muted-foreground text-center">
                <strong>Demo Login:</strong> worker@civicx.com / worker123
              </p>
            </div>
