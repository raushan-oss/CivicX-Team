...
const FirebaseService = {
  ...
  subscribeToReports(callback, filters = {}) {
    try {
      const { db } = import("@/lib/firebase-config").then(({ db }) => {
        let q = query(collection(db, "reports"), orderBy("createdAt", "desc"))
      ...
      return db // Return db instead of q to avoid undeclared variable error
    } catch (error) {
...
export const subscribeToReports = (callback, filters = {}) => {
  try {
    return FirebaseService.subscribeToReports(callback, filters)
  } catch (error) {
